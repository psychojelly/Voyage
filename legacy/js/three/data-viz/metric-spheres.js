...
export class MetricSpheres {
  ...
  update(data) {
    ...
    // Create summary spheres for the month
    const workouts = data.map(d => d.workout).filter(Boolean);
    const hearts = data.map(d => d.heart).filter(Boolean);
